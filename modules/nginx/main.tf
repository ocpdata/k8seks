  ...
  initContainers:
    - name: fix-nginx-agent-permissions
      ...
      command:
        - sh
        - -c
        - mkdir -p /etc/nginx/waf/bundles /etc/nginx/waf/nac-policies /etc/nginx/waf/nac-usersigs && touch /etc/nginx/waf/nac-usersigs/index.conf && chmod -R g+w /etc/nginx/waf
      volumeMounts:
